...
import { MarkPartialDeliveryCommand } from './mark-partial-delivery.command';

@Injectable()
export class MarkPartialDeliveryUseCase {
  constructor(
    @Inject(STORE_DELIVERY_REPOSITORY)
    private readonly deliveries: StoreDeliveryRepositoryPort,
  ) {}

  async execute(command: MarkPartialDeliveryCommand): Promise<void> {
    ...
    await this.deliveries.save(delivery.markPartial(command.observations, new Date()));
  }
}
